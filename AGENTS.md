...
THISCLOUD — self-hosted cloud platform ("Hypervisor OS") for VMs, networks, storage, apps. Monorepo: all five components live under the repo root. Detailed design reference: `CLAUDE.md`.

## Components

| Path | Stack | Role |
|---|---|---|
| `services/daemon/` | Rust (axum) | Daemon, HTTP API `:8080`, routes under `/api/v1` |
| `cli/` | Rust (clap) | `thiscloud` CLI → daemon over HTTP |
| `api/` | Go | Orchestrator bridge `:8081`, Terraform-provider-shaped `/api/v1/resources` CRUD |
| `web/` | Next.js 14 (App Router) | Dashboard; talks to Go API, **not** the daemon |
| `os/` | bash/kickstart | AlmaLinux 9 ISO pipeline (build, calamares, systemd, kickstart) |

`docs/api/openapi.yaml` is the single API contract (served by daemon, drives web-ui codegen, linted by spectral in CI).

## Commands

Rust workspace root is repo root (Cargo.toml; members `services/daemon`, `cli`). Go and web have their own roots.
...
- **Rust integration test pattern**: `services/daemon/tests/core.rs` is a thin `#[path]`-include module list. Add new integration tests as `tests/core/test_*.rs` **plus** a `mod` line in `tests/core.rs`, or they won't run.
...
- `api/` mirrors a Terraform provider: `internal/state` = tfstate-style store (persisted to `THISCLOUD_STATE_FILE`), `internal/backend` = HTTP client to daemon ("apply"), `internal/api` = generic CRUD. It never manages resources directly — the daemon does.
...
| `THISCLOUD_API_BIND` | `http://127.0.0.1:8081` | Go API bind |
